    ...
    while (tree) {
        ...
        switch (tree.name) {
            ...
            case "<":
                ...
                if (type && type2 && type != type2) {
                    errors++;
                    showError("", lineno, `Operands must be of the same type (line ${lineno})`, true);
                } else if (type === "array" || type2 === "array") {
                    errors++;
                    showError("", lineno, `Operands cannot be arrays (line ${lineno})`, true);
                }
                break;
...
module.exports = function (tree, _showError) {
    ...
    if (errors > 0) {
        ...
        console.log(cc.set("fg_green", "Compiled with 0 errors"))
    }
