...
module.exports = function (lexer, showError) {
  global.lexer = lexer;
    ...
    bnf: {
      ...
      func_declaration: [
        [
          ...
          `$$ = new Node($2.lineno,'FUNC_DECL', [$1, $4, $6], {name: $2[$2.name].name}); `,
        ],
      ],
      params: [
        [`param_list`, `$$ = $1;`],
      ...
      selection_stmt: [
        [`IF LPAREN expression RPAREN statement`, `$$ = new Node(yylineno,'SELECT_STMT', [$3, $5]);`],
        [`IF LPAREN expression RPAREN statement ELSE statement`, `$$ = new Node(yylineno,'SELECT_STMT', [$3, $5, $7]);`]
      ],
      iteration_stmt: [[
        `WHILE LPAREN expression RPAREN statement`, `$$ = new Node($3.lineno,'ITERATION_STMT',[$3, $5]);`
      ]],
      return_stmt: [
  ...
  parser.yy.parseError = function (err, hash) {
    if (!((!hash.expected || hash.expected.indexOf("';'") >= 0) && (hash.token === 'RBRACE' || hash.token === 'ERROR' || parser.yy.lineBreak || parser.yy.lastLineBreak || hash.token === 1))) {
      ...
      console.log(hash.text, message)
      throw new ParserError(showError(hash.text, hash.loc.first_line - 1, message, false, ""));
    }
  };
